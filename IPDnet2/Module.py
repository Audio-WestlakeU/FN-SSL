	...
	def forward(self, doa_gt, vad_gt, doa_est, vad_est, ae_mode, ae_TH=30, useVAD=True, vad_TH=[0.5,0.5]):
		...
		if self.source_mode == 'single':
			...
			for b_idx in range(nbatch):
				...
				acc[b_idx, :] = K_corr.sum(axis=0) / (K_gt.sum(axis=0)+0.000001)
				md[b_idx, :] = (K_gt.sum(axis=0) - K_corr.sum(axis=0)) / (K_gt.sum(axis=0)+0.000001)
				fa[b_idx, :] = (K_est.sum(axis=0) - K_corr.sum(axis=0)) / (K_gt.sum(axis=0)+0.000001)
...
# DPIPD for near_field
class DPIPD2(nn.Module):
	""" Complex-valued Direct-path inter-channel phase difference	
	"""
	def __init__(self, ndoa_candidate, mic_location, nf=257, fre_max=8000, ch_mode='M', speed=343.0):
		super(DPIPD2, self).__init__()
		self.ndoa_candidate = ndoa_candidate
		self.mic_location = mic_location
		self.nf = nf
		self.fre_max = fre_max
		...
		self.dpipd_template = self.data_adjust(dpipd_template_ori) # (nele, nazi, nf, nmic-1) / (nele, nazi, nf, nmic*(nmic-1)/2)
		del ITD, IPD
	def forward(self, source_doa=None, source_distance=None):
		# source_doa: (nb, ntimestep, 2, nsource) - [elevation, azimuth]
		# source_distance: (nb, ntimestep, nsource)
		mic_location = self.mic_location
		nf = self.nf
		fre_max = self.fre_max
		speed = self.speed
		if source_doa is not None and source_distance is not None:
			source_doa = source_doa.transpose(0, 1, 3, 2) # (nb, ntimestep, nsource, 2)
			nmic = mic_location.shape[-2]
			nb = source_doa.shape[0]
			nsource = source_doa.shape[-2]
			ntime = source_doa.shape[-3]
			ITD = np.empty((nb, ntime, nsource, nmic, nmic))  # Time differences, floats
			ITD1 = np.empty((nb, ntime, nsource, nmic, nmic))
			IPD = np.empty((nb, ntime, nsource, nf, nmic, nmic))  # Phase differences
			fre_range = np.linspace(0.0, fre_max, nf)

			# cal source location (x, y, z)
			source_xyz = np.stack([
				source_distance * np.sin(source_doa[..., 0]) * np.cos(source_doa[..., 1]),  # x
				source_distance * np.sin(source_doa[..., 0]) * np.sin(source_doa[..., 1]),  # y
				source_distance * np.cos(source_doa[..., 0])  # z
			], axis=-1)  # (nb, ntime, nsource, 3)
			for m1 in range(nmic):
				for m2 in range(nmic):
					dist_m1 = np.linalg.norm(source_xyz - mic_location[m1], axis=-1)  # (nb, ntime, nsource)
					dist_m2 = np.linalg.norm(source_xyz - mic_location[m2], axis=-1)  # (nb, ntime, nsource)
					ITD[:, :, :, m1, m2] = (dist_m2 - dist_m1) / speed  
					# cal IPD
					IPD[:, :, :, :, m1, m2] = -2 * np.pi * np.tile(fre_range[np.newaxis, np.newaxis, np.newaxis, :],
										[nb, ntime, nsource, 1]) * np.tile(ITD[:, :, :, np.newaxis, m1, m2], [1, 1, 1, nf])*(-1)  # !!!! delete -1
...
class PredDOA(nn.Module):
	def __init__(self,
		...
		self.gerdpipd = DPIPD2(ndoa_candidate=[res_the, res_phi],
	...
	def forward(self,pred_batch, gt_batch,idx,gt_batch_ipd=None,pred_batch_ipd=None,dir_name=None):
		pred_batch,_= self.pred2DOA(pred_batch = pred_batch, gt_batch = gt_batch,)
		metric = self.evaluate(pred_batch=pred_batch, gt_batch=gt_batch,idx=idx,gt_batch_ipd=gt_batch_ipd,pred_batch_ipd=pred_batch_ipd,dir_name=dir_name)
		return metric
		...
		if gt_batch is not None: 
			if type(gt_batch) is list:
				for idx in range(len(gt_batch)):
					if torch.is_tensor(gt_batch[idx]):
	...
	def pred2DOA_track(self, pred_batch=None, gt_batch=None, time_pool_size=None):
		...
		if pred_batch is not None:
            ...
            # 修改部分：使用MSE代替内积计算空间谱
            # 将pred_rebatch和dpipd_template调整为适合计算MSE的形状
			pred_rebatch_flat = pred_rebatch.contiguous().view(nb, nt, -1)  # (nb, nt, 2nf*(nmic-1))
			dpipd_template_flat = dpipd_template.contiguous().view(nb, nele, nazi, -1)  # (nb, nele, nazi, 2nf*(nmic-1))
            
            # 计算MSE
            # 扩展pred_rebatch_flat以便广播计算
			pred_rebatch_expanded = pred_rebatch_flat.unsqueeze(2).unsqueeze(2)  # (nb, nt, 1, 1, 2nf*(nmic-1))
            # 扩展dpipd_template_flat以便广播计算
			dpipd_template_expanded = dpipd_template_flat.unsqueeze(1)  # (nb, 1, nele, nazi, 2nf*(nmic-1))
            
            # 计算MSE
			mse = torch.mean((pred_rebatch_expanded - dpipd_template_expanded) ** 2, dim=-1)  # (nb, nt, nele, nazi)
            
            # 将MSE转换为相似度分数（MSE越小，相似度越高）
            # 使用负MSE或1/MSE等转换，这里使用负MSE
			pred_spatial_spectrum = mse
			# np.save('pred_ss',pred_spatial_spectrum.cpu().numpy())
			# print(aa)
			...
			for source_idx in range(self.max_num_sources):
				map = pred_spatial_spectrum  # (nb, nt, nele, nazi)

				max_flat_idx = map.reshape((nb, nt, -1)).argmin(2)
				...
				for b_idx in range(nb):
					for t_idx in range(nt):
						...
						mse_value = torch.mean(
                            (max_dpipd_template[b_idx, t_idx, :, :] - pred_rebatch[b_idx, t_idx, :, :]) ** 2)
                        # 将MSE转换为相似度分数
						ratio = mse_value  # MSE越小，ratio越接近1
	...
	def evaluate(self, pred_batch=None, gt_batch=None, vad_TH=[0.001, 0.4],idx=None,gt_batch_ipd=None,pred_batch_ipd=None,dir_name=None):
		"""
		evaluate the performance of DOA estimation
	    """
		ae_mode = ['azi']
		#doa_gt = gt_batch[0] 
		doa_gt =torch.cat((gt_batch[0][:,:,np.newaxis,:],gt_batch[0][:,:,np.newaxis,:]),dim=-2)
		# condition_to_map = (doa_gt >= 180.0)
		# doa_gt[condition_to_map] = doa_gt[condition_to_map] - 180.0  
		doa_est = pred_batch[0] * 180 / np.pi 
		vad_gt = gt_batch[-1].to(self.dev)
		dis_gt = gt_batch[-2].to(self.dev)
		vad_est = pred_batch[-2].to(self.dev)
		vad_est_max = vad_est.max()
		vad_est = vad_est / 0.2919
		ipd_template = gt_batch[-4].to(self.dev)
		mic_loc = gt_batch[-3]
		#print(doa_gt.shape,doa_est.shape,vad_gt.shape,vad_est.shape)
		metric = {}
		if idx != None:
			if dir_name is not None:  # 确保 dir_name 不是 None
				if not os.path.exists(dir_name):  # 检查目录是否存在
					os.makedirs(dir_name)  # 创建目录
				np.save(dir_name+str(idx)+'_doagt',doa_gt.cpu().numpy())
				np.save(dir_name+str(idx)+'_doaest',doa_est.cpu().numpy())
				np.save(dir_name+str(idx)+'_vadgt',vad_gt.cpu().numpy())
				np.save(dir_name+str(idx)+'_vadest',vad_est.cpu().numpy())
				np.save(dir_name+str(idx)+'_disgt',dis_gt.cpu().numpy())
				np.save(dir_name+str(idx)+'_mic',mic_loc)
				np.save(dir_name+str(idx)+'_template',ipd_template.cpu().numpy())
				#np.save(dir_name+str(idx)+'_ipd',pred_batch[-1].cpu().numpy())
				if gt_batch_ipd != None:
					np.save(dir_name+str(idx)+'_gt_ipd',gt_batch_ipd.cpu().numpy())
					np.save(dir_name+str(idx)+'_pred_ipd',pred_batch_ipd.cpu().numpy())
